...
describe("transfer.js", function () {
    ...
    function testTrs(name, trs, data, sender) {
        describe("returned transfer transaction " + name, function () {
            ...
            it("should be signed correctly", function (done) {
                ifmcoin.tx.verify(trs, sender, function (err) {
                    done(err);
                });
            });

            // it("should be deserialised correctly", function () {
            //     var deserialisedTx = ifmcoin.crypto.fromBytes(ifmcoin.crypto.getBytes(trs).toString("hex"));
            ...
            if (!data.secondSecret) {
                ...
                it("should be second signed correctly", function () {
                    var result = ifmcoin.tx.verifySecondSignature(trs, sender.secondPublicKey, trs.signSignature);
    ...
    //         var old_transaction = ifmcoin.transaction.createTransaction('AacRfTLtxAkR3Mind1XdPCddj1uDkHtwzD', 1, null, 'randomstring');
    ...
    //         // console.log("TXID " + ifmcoin.crypto.getId(old_transaction));
    //         // console.log("VERIFY " + ifmcoin.crypto.verify(old_transaction));
    //         // console.log("SIG " + old_transaction.signature + "\n");

    //         ifmcoin.crypto.verify(old_transaction).should.equal(true);
    ...
    //         ifmcoin.crypto.verify(old_transaction).should.equal(false);
